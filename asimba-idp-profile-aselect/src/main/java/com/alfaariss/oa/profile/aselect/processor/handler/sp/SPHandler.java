 ...
 * 
 */
package com.alfaariss.oa.profile.aselect.processor.handler.sp;
...
/**
 * A-Select Service Provider (SP) request handler.
 *
 * @author MHO
 * @author Alfa & Ariss
 *
 */
public class SPHandler extends AbstractAPIHandler
{
    /**
     * Creates the object.
     *
     ...
     */
    public SPHandler (IConfigurationManager oConfigurationManager, 
        ...
        String sProfileID) throws OAException
    {
        ...
        try
        {
            if (!_bEnabled)
            {
                _logger.info("SP handler: disabled");
                return;
            }
            
            _logger.info("Started: SP Handler");
        }
        catch (Exception e)
        {
     ...
     * <tr><td>uid</td><td>[uid]</td><td>true</td></tr>
        ...
        IBinding oBinding) throws ASelectException
    {
        ...
        try
        {
            ...
            String sAppId = (String)oRequest.getParameter(
                ASelectProcessor.PARAM_APPID);
            if (sAppId == null)
            {
                StringBuffer sbError = new StringBuffer("No '");
                sbError.append(ASelectProcessor.PARAM_APPID);
                sbError.append("' found in request");
                _logger.debug(sbError.toString());
                throw new ASelectException(
                    ASelectErrors.ERROR_ASELECT_SERVER_INVALID_REQUEST);
            ...
            String sAppUrl = (String)oRequest.getParameter(
                ASelectProcessor.PARAM_APPURL);
            if (sAppUrl == null)
            {
                StringBuffer sbError = new StringBuffer("No '");
                sbError.append(ASelectProcessor.PARAM_APPURL);
                sbError.append("' found in request");
                _logger.debug(sbError.toString());
                throw new ASelectException(
                    ASelectErrors.ERROR_ASELECT_SERVER_INVALID_REQUEST);
            ...
            else
            {
                ...
                sbDebug.append(ASelectProcessor.PARAM_FORCED_LOGON);
                sbDebug.append("' found in request: ");
                sbDebug.append(sForcedLogon);
            ...
            try
            {
                ...
                IRequestor oRequestor = _requestorPoolFactory.getRequestor(sAppId);
                if (oRequestor == null)
                {
                ...
                oSession = _sessionFactory.createSession(sAppId);
                ...
                oAttributes.put(ASelectProcessor.class, 
                    ASelectProcessor.SESSION_REQUESTOR_URL, sAppUrl);
            ...
            catch (ASelectException e)
            {
                ...
                oLogItem = new RequestorEventLogItem(null, null, null, 
                    ...
                    sAppId, this, "request=authenticate: " + sResultCode);
            }
            ...
            if (oLogItem == null)
                oLogItem = new RequestorEventLogItem(oSession, 
                    oServletRequest.getRemoteAddr(), 
                    RequestorEvent.AUTHN_INITIATION_SUCCESSFUL, this, 
                    "request=authenticate: SP");
        ...
        catch (Exception e)
        {
            if (oSession != null)
                oLogItem = new RequestorEventLogItem(oSession, 
                    oServletRequest.getRemoteAddr(), 
                    RequestorEvent.INTERNAL_ERROR, this, "request=authenticate");
            else
                oLogItem = new RequestorEventLogItem(null, null, 
        ...
        IBinding oBinding) throws ASelectException
    {
        ...
        try
        {
            ...
            {
                _logger.debug("No session attribute found with name: " 
                ...
                switch (oSession.getState())
                {
                    ...
                    {                       
                        ...
                        IRequestor oRequestor = 
                            _requestorPoolFactory.getRequestor(oSession.getRequestorId());
                        if (oRequestor == null)
                        {
                            _logger.debug("No Requestor found with id: " + oSession.getRequestorId());
                            throw new ASelectException(
                                ASelectErrors.ERROR_ASELECT_INTERNAL_ERROR);
                        }
                        ...
                        {
                            _logger.debug(
                                "No Requestor Pool found for requestor id: " 
                        ...
                        if (doSigning(oRequestorPool, oASRequestorPool, oRequestor))
                        {
                            ...
                            if (!verifySignature(sSignature, 
                                oRequestor.getID(), sbSignature.toString()))
                            {
                                _logger.error(
                                    "Invalid signature for request from requestor with id: " 
                                    + oRequestor.getID());
                    ...
                    case AUTHN_FAILED:
                    {
                        sResultCode = ASelectErrors.ERROR_ASELECT_AUTHSP_COULD_NOT_AUTHENTICATE_USER;
                        break;
                    }
                    ...
                    case USER_BLOCKED:
                    {
                        sResultCode = ASelectErrors.ERROR_USER_BLOCKED;
                        break;
                    }
                    case USER_UNKNOWN:
                    {
                        sResultCode = ASelectErrors.ERROR_ASELECT_UDB_UNKNOWN_USER;
                        break;
                    }
                    case PASSIVE_FAILED:
                    {
                        sResultCode = ASelectErrors.ERROR_PASSIVE_FAILED;
                        break;
                    }
                    default:
                    {
                        sResultCode = ASelectErrors.ERROR_ASELECT_INTERNAL_ERROR;
                        break;
                    }
                }
            }
            catch (ASelectException e)
            {
                ...
                oLogItem = new RequestorEventLogItem(
                    oSession,oServletRequest.getRemoteAddr(),
                    RequestorEvent.REQUEST_INVALID, this, 
                    "request=verify_credentials: " + sResultCode);
            }
            oResponse.setParameter(
                ASelectProcessor.PARAM_RESULT_CODE, sResultCode);
            oResponse.setParameter(
                ASelectProcessor.PARAM_ASELECTSERVER, _OAServer.getID());
            
            if (oLogItem == null)
                oLogItem = new RequestorEventLogItem(oSession, 
                    oServletRequest.getRemoteAddr(), 
                    RequestorEvent.TOKEN_DEREFERENCE_SUCCESSFUL, this, 
                    "request=verify_credentials : SP");
        ...
        try
        {
            ...
            try
            {
                ...
                if (doSigning(oRequestorPool, oASRequestorPool, oRequestor))
                {
                    ...
                    if (sAppUrl != null)
                        htSignatureData.put(ASelectProcessor.PARAM_APPURL, sAppUrl);
                ...
                if (!_aliasStoreSPRole.isAlias(
                    BrowserHandler.ALIAS_TYPE_CREDENTIALS, sAppId, sCredentials))
                {
                ...
                {//DD if no app_url is supplied as return url, use the profile url as return url
                    sReturnURL = sAsUrl;
                }
                ...
                ISessionAttributes oAttributes = oSession.getAttributes();
                oAttributes.put(ASelectProcessor.class, 
                    ASelectProcessor.SESSION_REQUESTOR_URL, sReturnURL);
            ...
            catch (ASelectException e)
            {
                ...
                oLogItem = new RequestorEventLogItem(null, null, null, 
                    RequestorEvent.REQUEST_INVALID, null, 
                    oServletRequest.getRemoteAddr(), 
                    sAppId, this, "request=slo: " + sResultCode);
            }
