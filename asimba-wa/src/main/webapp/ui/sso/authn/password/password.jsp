...
<fmt:setBundle 
	basename="com.alfaariss.oa.authentication.password.resources.warnings" 
	var="warnings" scope="request"/> 
<fmt:setBundle 
	basename="com.alfaariss.oa.authentication.password.resources.messages" 
	var="messages" scope="request"/>
<fmt:setBundle 
	basename="com.alfaariss.oa.authentication.password.resources.errors" 
			...
			<div id="content">			
				...
				<div id="contentMain">					
					<form id="passwordForm" class="oaForm" action="${pageContext.request.contextPath}/sso/web" method="post" name="login">				
						<input type="hidden" name="asid" value='<c:out value="${requestScope.asid.id}"/>'>
						<fieldset>
	  						<legend>
	  							<c:choose>
									<c:when test="${requestScope.user_id == null}">
										<fmt:message bundle="${messages}" 
											key="username_credentials_message"/>
									</c:when>
									<c:otherwise>
										<fmt:message bundle="${messages}" 
											key="credentials_message"/>
									</c:otherwise>
								</c:choose>
							</legend>					
							<c:if test="${requestScope.user_id == null}">
							 ...
							 <div>
								<label><fmt:message bundle="${messages}" key="password_label"/></label>		
